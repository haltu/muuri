(function (window) {
  ...
  QUnit.test('getItems: should return the instance`s items', function (assert) {
    ...
    window.requestAnimationFrame(function () {
      assert.deepEqual(grid.getItems('positioning').map(item => item._id), [items[6], items[7]].map(item => item._id), 'should allow providing "positioning" state as the first argument');
    });
