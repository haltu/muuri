(function (window) {
  ...
  QUnit.test('show: should accept an item, element or an index (or an array of them) as the first argument', function (assert) {
    ...
    grid.show(0);
    assert.deepEqual(grid.getItems('visible'), items.slice(0, 1), 'should accept an index as the first argument');

    grid.show(items[1]);
    assert.deepEqual(grid.getItems('visible'), items.slice(0, 2), 'should accept an item as the first argument');

    grid.show(items[2].getElement());
    assert.deepEqual(grid.getItems('visible'), items.slice(0, 3), 'should accept an element as the first argument');

    grid.show([3, items[4].getElement(), items[5]]);
    assert.deepEqual(grid.getItems('visible'), items.slice(0, 6), 'should accept an array of items, elements and indices as the first argument');
  ...
  QUnit.test('show: should call the onFinish callback once the animation is finished', function (assert) {
    ...
    .show(0, {onFinish: function (completedItems) {
      ...
      assert.deepEqual(completedItems, items.slice(0, 1), 'callback: should receive the shown items as it`s first argument');
